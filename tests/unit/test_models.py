...
from src.shared.models import Entity, AudioMetadata, SpeakerInfo, WordDatabase
    ...
    def test_quality_score_validation(self):
        ...
        for score in [0.0, 0.5, 1.0]:
            Entity(
                ...
                start_time=1.0, end_time=2.0, duration=1.0,
                confidence=0.9, probability=0.9, syllables=["test"],
                syllable_count=1, quality_score=score, speaker_id="speaker_0",
                recording_id="test", recording_path="test.wav", processed=False,
                created_at=datetime.now().isoformat()
            )
    ...
    def test_syllable_count_consistency(self):
        ...
        with pytest.raises(ValueError, match="syllable_count must match length of syllables"):
            Entity(
                ...
                start_time=1.0, end_time=2.0, duration=1.0,
                confidence=0.9, probability=0.9, syllables=["ho", "la"],
                syllable_count=3, quality_score=0.8, speaker_id="speaker_0",
                recording_id="test", recording_path="test.wav", processed=False,
                created_at=datetime.now().isoformat()
            )
...
class TestWordDatabase:
    ...
    def test_get_entities_by_speaker(self):
        ...
        entity2 = Entity(
            ...
            syllable_count=1, quality_score=0.8, speaker_id="speaker_1",
            recording_id="test", recording_path="test.wav", processed=False,
            created_at=datetime.now().isoformat()
        )
        ...
        # Filter by speaker
        speaker0_entities = db.get_entities_by_speaker("speaker_0")
        speaker1_entities = db.get_entities_by_speaker("speaker_1")
        
        assert len(speaker0_entities) == 1
        assert len(speaker1_entities) == 1
        assert speaker0_entities[0].speaker_id == "speaker_0"
        assert speaker1_entities[0].speaker_id == "speaker_1"
    ...
    def test_get_entities_by_confidence(self):
        ...
        low_conf_entity = Entity(
            ...
            syllable_count=1, quality_score=0.3, speaker_id="speaker_0",
            recording_id="test", recording_path="test.wav", processed=False,
            created_at=datetime.now().isoformat()
        )
