...
def test_speaker_integration_e2e():
    ...
    entities = [
        Entity(
            ...
            phonetic="ˈoʊlə", quality_score=0.8, speaker_id="speaker_0",
        ...
        Entity(
            ...
            phonetic="maˈɾi.a", quality_score=0.8, speaker_id="speaker_0",
            recording_id="test", recording_path="test.wav", processed=False,
            clip_path=None, selection_reason=None, created_at="2025-08-15T12:00:00Z"
        )
    ]
    ...
    speaker_mapping = [
        {"start": 0.0, "end": 5.0, "speaker": "María", "speaker_id": "speaker_1"},
        {"start": 5.0, "end": 15.0, "speaker": "Carlos", "speaker_id": "speaker_2"}
    ]
    ...
    assert updated_entities[0].speaker_id == "speaker_1"  # María (0-5s range)
    assert updated_entities[1].speaker_id == "speaker_2"  # Carlos (5-15s range)
    
    # Validate speaker map
    assert "speaker_1" in speaker_map
    assert "speaker_2" in speaker_map
    assert speaker_map["speaker_1"]["name"] == "María"
    assert speaker_map["speaker_2"]["name"] == "Carlos"
...
def test_default_speaker_handling_e2e():
    ...
    assert updated_entities[0].speaker_id == "speaker_0"
    assert "speaker_0" in speaker_map
    assert speaker_map["speaker_0"]["name"] == "Test Speaker"